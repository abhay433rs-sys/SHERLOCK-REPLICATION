...
#    row0-PV, row0-IV, row0-IO, row1-PV, row1-IV, row1-IO, ...
sentences = []
...
def tokens_to_words_aligned(sentence, sentence_scores):
    ...
    sentence_scores: list of (token, surprisal) tuples from minicons

    Aligns BPE tokens to the sentence's actual whitespace-split words by
    greedily accumulating tokens (stripped of the Ġ marker) until the
    reconstructed string matches each target word. This avoids relying
    solely on the Ġ marker, which can be inconsistent for all-caps text,
    single-letter words, or unusual merges.
    """
...
results_df.to_csv(r"E:\Project LLM\Model_test\mistral-7b_V0.3_SURPRSE.csv", index=False)
print("Saved word-level surprisal results (grouped by triplet) to mistral-7b_V0.3_SURPRSE.csv")